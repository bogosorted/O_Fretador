...
public class FadeSystemBehaviour : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        CreateFade(true,Random.Range(0.3f,5),Random.Range(0,TypesF.Length));
    }
    /// <summary>      
    ...
    public void CreateFade(bool in_or_out = true, float seconds = 1f, int type_fade = 0)
    {
        ...
        currentFade.GetComponent<Animator>().SetBool("Ativator", in_or_out);
        if (seconds != 0)
            currentFade.GetComponent<Animator>().SetFloat("Seconds", 1 /seconds);
    }
}
